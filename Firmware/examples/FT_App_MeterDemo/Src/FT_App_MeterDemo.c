...
char *info[] = { "FT800 Meter Dial Application",
...
static void rotate_around(ft_int16_t x, ft_int16_t y, ft_int16_t a,ft_int16_t handle)
{
  Ft_Gpu_CoCmd_LoadIdentity(phost);
  Ft_Gpu_CoCmd_Translate(phost,F16(x),F16(y));
  Ft_Gpu_CoCmd_Rotate(phost,a);
  Ft_Gpu_CoCmd_Translate(phost,F16(-x),F16(-y));
...
static ft_void_t scroller_run()
{
	  ft_int32_t change;
	  ft_uint32_t sx;
	  ft_int32_t sxtemp=0,sxprev=0,dragdeg=0;
...
//	Check if the Play key and change the color
	...
	Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(FT_DispWidth*16,0));
	Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(FT_DispWidth*16,FT_DispHeight*16));
#else
	...
	Ft_App_WrCoCmd_Buffer(phost, BITMAP_HANDLE(0));
	Ft_App_WrCoCmd_Buffer(phost, CELL(0));
	Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(((FT_DispWidth/2)-14+10)*16,(FT_DispHeight-77+5)*16));
	Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(((FT_DispWidth/2)-2+15)*16,(FT_DispHeight-67+8)*16));
	Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(((FT_DispWidth/2)-14+10)*16,(FT_DispHeight-57+11)*16));
	Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(((FT_DispWidth/2)-14+10)*16,(FT_DispHeight-77+5)*16));
	//Ft_App_WrCoCmd_Buffer(phost, END());
#else
	Ft_App_WrCoCmd_Buffer(phost, VERTEX2II(((FT_DispWidth/2)-14+10),(FT_DispHeight-77+5),0,0));
...
*/
	...
	ft_int32_t FileLen=0,fsize=0;
		...
		if(Handle== 0)
		{
			...
			Ft_Gpu_Hal_WrCmd32(phost,  0/*RamAddr*/);
			#if  defined(MSVC_PLATFORM) || defined(MSVC_FT800EMU) || defined(FT900_PLATFORM)
			Ft_Gpu_Hal_WrCmdBuf(phost,SAMAPP_Abs_Base_Dial_L1,/*sizeof(SAMAPP_Base_Dial_L1)*/SAMAPP_Abs_BaseDial_Size146x146_L1);
			#endif
			#ifdef ARDUINO_PLATFORM
		...
		{
			Ft_Gpu_Hal_WrCmd32(phost,  CMD_INFLATE);
			Ft_Gpu_Hal_WrCmd32(phost,  15000/*RamAddr*/);
			...
			Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(15000));
...
#ifdef DISPLAY_RESOLUTION_WVGA
			Ft_App_WrCoCmd_Buffer(phost, BITMAP_LAYOUT_H(Stride>>10,Height>>9));
			Ft_App_WrCoCmd_Buffer(phost, BITMAP_SIZE_H(Width>>9,Height>>9));
#endif
			//Ft_App_WrCoCmd_Buffer(phost, VERTEX2II(150,0,Handle,0));		
		}		
...
ft_void_t Abs_Meter_Dial()
{
...
#elif defined(DISPLAY_RESOLUTION_QVGA) || defined(DISPLAY_RESOLUTION_HVGA_PORTRAIT)
	Sx1 = Sy1 = 130;
	ButY = 1;
	ButWid = 23;
	NoY = 12;
	...
	CircleX = ScreenWidthMid;
	CircleY = ScreenHeightMid;
	ButY = FT_DispHeight - 462;
	ButWid = 45;
	NoY = (FT_DispHeight-462 + 42/2);
	...
	do{
		...
		Ft_App_WrCoCmd_Buffer(phost, SCISSOR_SIZE(Sx1,Sy1) );	
		...
		Ft_App_WrCoCmd_Buffer(phost, LINE_WIDTH(1*16));
		Ft_App_WrCoCmd_Buffer(phost, STENCIL_OP(INCR,INVERT) );
		...
		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(Tx1*16,FT_DispHeight*16));
		//Ft_App_WrCoCmd_Buffer(phost, VERTEX2II(Tx2,Ty2,0,0));
		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(Tx2*16,Ty2*16));
		//Ft_App_WrCoCmd_Buffer(phost, VERTEX2II(Tx3,FT_DispHeight,0,0));
		Ft_App_WrCoCmd_Buffer(phost, VERTEX2F(Tx3*16,FT_DispHeight*16));
...
ft_void_t Relative_Meter_Dial()
{
	...
	ft_int16_t OuterCircleRadius,PrevValDeg=0,CircleX,CircleY,ButY,ButWid,NoY,Delta,G1,G2,Sx1,Sy1,X2,Y2;
	...
	ft_int32_t change;
	ft_int32_t sx,storesx,storepresx;	
...
CircleX = ScreenWidthMid;
CircleY = ScreenHeightMid;
ButY = FT_DispHeight -460;
ButWid = 150;
NoY = (FT_DispHeight-460 + 30/2);
#endif
	...
	do{
		...
		Ft_Gpu_CoCmd_Gradient(phost,0, FT_DispHeight, 0x1E1E1E,ScreenWidthMid-60, ScreenHeightMid-21, 0x999999);

	Ft_App_WrCoCmd_Buffer(phost,COLOR_RGB(0,0,0));

	Ft_App_WrCoCmd_Buffer(phost,BEGIN(LINES));

	Ft_App_WrCoCmd_Buffer(phost,VERTEX2F((0)*16,(30)*16 ));
	Ft_App_WrCoCmd_Buffer(phost,VERTEX2F(FT_DispWidth*16,154 *16));
	  ...
	  drag = scroller.base>>4;
		...
		ft_uint8_t statusButtonH = 20, statusButtonW = 30, buttonsPerRow = 6, rowGap = 10, curRow = 0;
		ft_uint16_t edgeOffset = 30, startingX = 10 , startingY = FT_DispHeight - statusButtonH - edgeOffset; 

		ft_uint16_t buttonVerticalGap = (FT_DispHeight - edgeOffset * 2 - buttonsPerRow * statusButtonH) / (buttonsPerRow - 1);
