...
*****************************************************************************/
/**
* @file                          FT_App_Gauges.c
* @brief                          Sample application to demonstrate FT800 primitives and built-in widgets

version 3.2 - July/29/2015 - Added support for FT90x and FT80x platforms.
...
#define WRITE2CMD(a) Ft_Gpu_Hal_WrCmdBufFromFlash(phost,a,sizeof(a))
...
void Gauges()
{
  ft_uint16_t w,h,n, a,th,size_d,temp[1024],tval;
  ft_int32_t rval,tgt;
  ft_uint8_t i,bi,z,buff[512];
  ft_int32_t x,y,tx,ty,o,dt;
...
// load the deflated icons to GRAM via J1
  ...
  Ft_Gpu_Hal_WrCmd32(phost,20*1024L);
  WRITE2CMD(digits);

 Ft_Gpu_CoCmd_Dlstart(phost);
  Ft_Gpu_CoCmd_SetFont(phost,13,20*1024l);
  Ft_App_WrCoCmd_Buffer(phost,BITMAP_HANDLE(13));
#ifdef DISPLAY_RESOLUTION_QVGA
  Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(20480L+144 - (32L*(21*60))));
  Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(L4,21,60));
  Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, 42, 60));
#else
  Ft_App_WrCoCmd_Buffer(phost,BITMAP_SOURCE(20480L+144 - (32L*(31*87))));
  Ft_App_WrCoCmd_Buffer(phost,BITMAP_LAYOUT(L4,31,87));
  Ft_App_WrCoCmd_Buffer(phost,BITMAP_SIZE(NEAREST, BORDER, BORDER, 62, 87));
  #endif  
...
// Construct the Gauge background	
 ...
 Ft_Gpu_CoCmd_Dlstart(phost);   							// dl = 0;
...
// Ft_App_WrCoCmd_Buffer(phost,VERTEX2II(0,0,2,0));
 ...
 for(z=0;z<(FT_DispWidth/w);z++)
 {
   ...
   if(z==0)Ft_Gpu_CoCmd_Text(phost,ox,h-10,28,OPT_CENTERX,"X Position");
#else
...
// copy the displaylist from DLRAM to GRAM 
 ...
 do
 {
   ...
   for(z=0;z<(FT_DispWidth/w);z++)
   {
     ...
     {
	#ifdef FT_801_ENABLE
	val = Ft_Gpu_Hal_Rd32(phost,REG_CTOUCH_TOUCH0_XY);		
	xvalue = ((val>>16));

	if(xvalue!=-32768)                                                 
     {  
            val = (xvalue * 899.0 / FT_DispWidth);
     }else
		val = 0;

      #else
	val = Ft_Gpu_Hal_Rd32(phost,REG_TOUCH_RZ);	
      #endif			
	val = 10*min(899,val);			
     }else   
     if(z==1)
...
char* const info[] PROGMEM = {  "FT800 Gauges Application",

                          "APP to demonstrate interactive Gauges,",
                          "using Lines & custom Font."
                       }; 
